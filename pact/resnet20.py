...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, bitW, bitA):
        ...
        self.bitA = bitA
        self.alpha0 = nn.Parameter(torch.tensor(10.))
    ...
    def forward(self, x):
        out = self.res1(self.pact(self.res0(x), self.alpha0, self.bitA))
        return self.pact(out + self.shortcut(x), self.alpha1, self.bitA)
...
if __name__ == "__main__":
    ...
    net = NaiveResnet20(7,bitW=4,bitA=6)
    out = net(data)
    print(net)
    print(data.shape)
    print(out.shape)