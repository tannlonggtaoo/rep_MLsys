...
from naivemodel import NaiveResnet
...
if __name__ == '__main__':
    model = NaiveResnet(n_class=10,quantizable=True)
    ckpt_path = './Resnet/models/original'
    ckpt, _ = loadparam(ckpt_path)
    model.load_state_dict(ckpt)

    train_loader, test_loader = getloader(128,0)
    calib_loader = DataLoader(Subset(train_loader.dataset,list(range(10000))),128)
